    ...
    */
    ...
    public void loadCreatureSprites() {
        ...
        for (int i=0; i<4; i++) {
            ...
            for(int x=0; x<enemies;x++)
            	...
            	if(s.getArchType(x).compareTo("balloon")==0)
                	...
                	if(s.getArchType(x).compareTo("Blunt")==0)
                		enemyAnim[x][i] = createFlyAnim(
                			images[i][imageIndex++], images[i][imageIndex++], images[i][imageIndex++]);
                else
                    if(s.getArchType(x).compareTo("shark")==0)
                    	enemyAnim[x][i] = createFlyAnim(
                    		images[i][imageIndex++], images[i][imageIndex++], images[i][imageIndex++]);
        ...
        for(int x=0; x<enemies;x++)
            ...
            if(s.getArchType(x).compareTo("Blunt")==0)
            		enemySprites[x] = new Blunt(enemyAnim[x][0], enemyAnim[x][1],
            				enemyAnim[x][2], enemyAnim[x][3]);
            else
            if(s.getArchType(x).compareTo("shark")==0)
                	enemySprites[x] = new Shark(enemyAnim[x][0], enemyAnim[x][1],
                			enemyAnim[x][2], enemyAnim[x][3]);
